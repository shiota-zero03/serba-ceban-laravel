...
<div class="row gap-4">
    <div class="col-12">
        <h4 class="m-md-0 text-md-start text-center">Edit Pemesanan</h4>
    </div>
    <div class="col-12">
                            ...
                            <div class="form-group">
                                ...
                                <select name="mitra_id" id="mitra_id" class="form-control">
                                    ...
                                    @foreach ($mitra as $user)
                                        <option @selected($data['mitra_id'] == $user->id) value="{{ $user->id }}">{{ $user->name }} - {{ $user->phone_number }}</option>
                                    @endforeach
                                </select>
                                @error('mitra_id')
    ...
    <script>
        ...
        function getTable () {
            ...
            if(mitraId) {
                ...
                let orderedProducts = @json($data->detail->pluck('jumlah_pesan', 'produk_id')->toArray());
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (data.produk.length > 0) {
                            data.produk.forEach(product => {
                                let isChecked = orderedProducts.hasOwnProperty(product.id) ? 'checked' : '';
                                let jumlahPesanan = orderedProducts[product.id] || '';
                                tableBody.append(`
                                    <tr>
                                        <td class="text-center">
                                            <input type="checkbox" name="produk_id[]" value="${product.id}" class="produk-checkbox" ${isChecked}>
                                        </td>
                                        <td>${product.kode_produk}</td>
                                        <td>${product.nama_produk}</td>
                                        <td>Rp ${product.harga.toLocaleString('id-ID')}</td>
                                        <td>
                                            <input type="number" name="jumlah[${product.id}]" class="form-control" min="1" value="${jumlahPesanan}" ${isChecked ? '' : 'disabled'}>
                                        </td>
                                    </tr>
                                `);
                            });
