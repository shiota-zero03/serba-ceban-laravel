...
<div class="row gap-4">
    <div class="col-12">
        <h4 class="m-md-0 text-md-start text-center">Detail Penjualan</h4>
    </div>
    <form action="{{ route('penjualan.update', $dataPemesanan['id']) }}" class="col-12" method="POST">
        @csrf
        @method('PUT')
        <div class="card mb-3">
            <div class="card-body">
                            ...
                            <table class="table table-bordered table-hovered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="min-width: 120px">Jumlah Barang Terjual</th>
                                        <th style="min-width: 120px">Sisa Barang</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @if(count($dataPemesanan['detail']) > 0)
                                        @foreach ($dataPemesanan['detail'] as $key => $detail)
                                            <tr>
                                                    ...
                                                    @else
                                                        @if($hariIni->between($tanggalPenerimaan, $batasAkhir))
                                                            <input
                                                                ...
                                                                data-jumlah-pesan="{{ $detail->jumlah_terima }}"
                                                                data-id="{{ $detail['id'] }}">
                                                        @else
                                                            {{ $detail->jumlah_terjual }}
                                                        @endif
                                                ...
                                                <td>
                                                    @if($dataPemesanan['is_paid'])
                                                        {{ $detail->jumlah_terima - $detail->jumlah_terjual }}
                                                    @else
                                                        @if($hariIni->between($tanggalPenerimaan, $batasAkhir))
                                                            <input readonly type="number" name="selisih[{{ $detail['id'] }}]" class="form-control selisih" min="0"
                                                                value="{{ $detail->jumlah_terima - $detail->jumlah_terjual }}" id="selisih-{{ $detail['id'] }}">
                                                        @else
                                                            {{ $detail->jumlah_terima - $detail->jumlah_terjual }}
                                                        @endif
                                                    @endif
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="7" class="text-center small">Tidak ada produk ditemukan</td>
                                        </tr>
                                    @endif
                                </tbody>
                            </table>
